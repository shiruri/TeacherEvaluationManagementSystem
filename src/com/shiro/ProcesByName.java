...
class ProcesByName implements ProcessLoginBy{
	...
	public boolean process(String input,String password,Admin an) throws SQLException, ClassNotFoundException {
		...
		byte[] Salt = null;
		PreparedStatement getSalt = con.prepareStatement(" SELECT adminSalt FROM admin LEFT JOIN adminhash  ON admin.adminId = adminhash.adminId WHERE adminName  = ?;");   
		...
		String hashedPassword = c.getSecurePassword(password,Salt);


		PreparedStatement statement = con.prepareStatement("SELECT adminName,adminPassword FROM admin LEFT JOIN adminhash  ON admin.adminId = adminhash.adminId WHERE adminName  = ?;");   
		statement.setString(1,input);

		ResultSet res = statement.executeQuery();

		while(res.next()) {
			RetrievedEmail = res.getString("adminName");
	...
	public static void setCurrentAdmin(String RetrievedEmail,Admin an) throws ClassNotFoundException, SQLException {
		...
		con = DatabaseManagement.getCon();
		stmt = con.createStatement();
		PreparedStatement statement = con.prepareStatement("SELECT adminName,adminId from admin where adminName = ?");   
